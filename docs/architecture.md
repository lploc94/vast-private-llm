...
- **Disk:** `VASTLLM_DATA_DIR` defaults to `./data`; it holds `app.db`, the saved Vast key, and `known_hosts`. The dashboard saves a Vast key with owner-only permissions. Keep the directory private and use one dashboard process per data directory.
