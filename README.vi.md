...
Destroy instance không tự gỡ SSH key. Sau khi tài khoản Vast không còn instance nào, bấm **Thu hồi SSH key do app quản lý** trong tab Deploy. App gỡ public key tương ứng khỏi tài khoản Vast hiện đang chọn, xác nhận đã gỡ rồi mới xóa cặp key trong `<data-dir>/ssh/`; lần deploy sau sẽ tạo key mới. Vast API key cần `user_read` và `user_write`. Nút này không gỡ key đặt qua `VASTLLM_SSH_KEY_PATH`. Nếu Vast không còn key tương ứng trong tài khoản đang chọn, app giữ key local và yêu cầu bạn kiểm tra có đổi tài khoản hoặc đã gỡ key từ Vast Console hay chưa. Việc xóa key cấp tài khoản không chứng minh key đã bị tháo khỏi một instance khác đang chạy; hãy kết thúc mọi instance trước.
